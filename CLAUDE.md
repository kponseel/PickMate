...
PowerShell). Le but immédiat : **flasher et lancer GamesHub** (plateforme de
jeux de soirée multi-téléphone, hors-ligne, hébergée par l'ESP32 et pilotée
par le Flipper).

## C'est quoi ce dépôt
Deux sous-projets reliés par une liaison série :
- `esp32-hub/` — serveur de quiz **ESP32-S2** (PlatformIO / Arduino). Code : `esp32-hub/src/main.cpp`.
- `apps/gameshub/` — app **Flipper Zero** (`.fap`, C) qui pilote le jeu en UART.
...
- **Sans `uploadfs`**, la page joueur est vide et `/admin` 404. À refaire à chaque
  modif sous `esp32-hub/data/`.
- À ce stade, testable sans le Flipper : téléphone → Wi-Fi `Paris - Mini Jeux Gratuits` → page de jeu.
...
- Téléphone → Wi-Fi **`Paris - Mini Jeux Gratuits`** → la page s'ouvre seule (sinon `http://192.168.4.1`).
...
- Si le build PlatformIO échoue sur un **tag de lib manquant**, enlever le suffixe `#vX.Y.Z` dans `esp32-hub/platformio.ini`.
